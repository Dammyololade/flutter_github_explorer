  ...
  group("SearchRepositoryImpl", () {
    test("should get search results from remote datasource", () async {
      ...
      when(() => searchRemoteDataSource.search(any())).thenAnswer(
          (_) async => ApiResultModel.success(data: {
      ...
      verify(() => searchLocalDatasource.cacheSearchedData(any(), any())).called(1);
      expect(response, isA<ApiResultModelSuccess>());
      final parsedResponse = response as ApiResultModelSuccess<SearchResponse>;
      expect(parsedResponse.data.model.items.isNotEmpty, true);
